...
const handler = async (req: Request) => {
  ...
  if (!apiKey) {
    return Response.json(
      { ok: false, error: "tavily_not_configured" },
      { status: 503 },
    );
  }

  const tav = await fetch(TAVILY_ENDPOINT, {
    method: "POST",
  ...
  if (!tav.ok) {
    const text = await tav.text();
    return Response.json(
      { ok: false, error: "tavily_failed", status: tav.status, body: text },
      { status: 502 },
    );
  }
  const data = (await tav.json()) as unknown;
